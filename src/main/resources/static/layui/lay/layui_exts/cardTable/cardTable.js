layui.define(['table', 'laypage','jquery', 'element'], function(exports) {
	...
	function postData(url,option) {
		var po = option.where;
			po['limit']=option.limit;
			po['page']=option.currentPage;
		$.ajax({
		      type: "post",
		      url: url,
		        dataType : "json",
		        ...
		        success: function (d) {
					var retData = initData(d, option);
					if (!d.data) {
						option.data = [];
						option.count = 0;
		            } else {
						option.data = retData.data;
						option.count = retData.data.length;
					}
					var html = "";
					// 根据结果进行相应结构的创建
	...
	var tt = {
		...
		reload: function (id, opt) {
			if(_instances[id]&&_instances[id].option){
