...
function objectData(att){
	var instanceAtt={};
	for(let xx in att){
	    instanceAtt[att[xx].name]=att[xx].nodeValue;
	}
...
//	parentShow(genurl,att.name,att.id)
}
...
//					var di = doc.createElement(reli);
					for(var key in relationData){
						...
						var propName=relationData[key].prop
						if(!propName){
							propName=''
						}
						graph.insertEdge(parent, null, propName, relNode, endi);
					}
				}
			}
			finally
