...
@Service
public class CypherShowService extends LayUIDomain{
    @Autowired
    ...
    private Map<String, Boolean> formFieldTemplate(Model model, Map<String, Object> po, String[] columnArray,
	    String[] headers, List<Map<String, String>> customType) {
	Map<String, Boolean> boolMap = new HashMap<>();
	JSONObject vo = new JSONObject();
	String tableName = code(po);
	...
	boolMap.put(HAS_DATE_FIELD, hasDateField);
	Map<String, Boolean> layUseInfo = customFieldHandle(model, sb, customType, null);
	boolMap.putAll(layUseInfo);
	sb.append("</div>");
	appendSubmitResetBtn(sb);
	// 总的表单内容
	...
	if ("textEditor".equalsIgnoreCase(showType)) {
	    ...
	    textEditorJs.append("\n if(data." + code + "!=undefined){\n" + "$('#" + code + "').val(data." + code + ");\n"
		   +" layedit.set({uploadImage: {url: '"+LemodoApplication.MODULE_NAME+"/file/uploadImage',type:'post'}});\n"
     ...
     * @return
     */
    private Map<String, Boolean> customFieldHandle1(Model model, List<Map<String, String>> customType,
	    List<Map<String, Object>> selectOptions) {
	Map<String, Boolean> fieldUseJsMap = new HashMap<>();
	StringBuilder js = new StringBuilder();
	StringBuilder switchJs = new StringBuilder();
    ...
    private boolean fieldHandle(String[] columnArray, String[] headers, Map<String, Map<String, Object>> fiMap,
	    Map<String, Map<String, Object>> fivMap, StringBuilder sb, List<Map<String, String>> customType) {
	boolean hasDateField = false;
	for (int i = 0, k = 0; i < headers.length; i++) {
	    // 收集自定义字段信息
	    String columni = columnArray[i];
	...
	if ("textEditor".equalsIgnoreCase(showType)) {
	    fieldString = textArea(name, code, validator, getReadOnly(ctypei));
	    js.append(" layedit.set({uploadImage: {url: '"+LemodoApplication.MODULE_NAME+"/file/uploadImage',type:'post'}});\n");
	    js.append(" var " + code + "Index = layedit.build('" + code + "');\n");
	}
	if ("line".equalsIgnoreCase(showType)) {
	    fieldString = layFormLine(code, name, null);
	...
	sb.append(" <label  class=\"layui-form-label\" >" + name + "</label>");
	sb.append(" <div class=\"layui-input-block\">");
	sb.append("       <textarea id=\"" + code + "\" name=\"" + code
		+ "\" class=\"layui-textarea\"  lay-verify=\""+code+"\" onscroll=\"this.rows++;\"");
	addValidator(validator, sb);
	...
	String entityString = "match (n:DbTable) -[r]->(e:layTableToolOpt) where id(n)=" + id(tableMap)
		+ " return e";
