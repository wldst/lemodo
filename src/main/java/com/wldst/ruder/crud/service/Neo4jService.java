...
@Service
public class Neo4jService extends DataBaseDomain {

    final static Logger debugLog = LoggerFactory.getLogger("debugLogger");
    final static Logger logger = LoggerFactory.getLogger(Neo4jService.class);
     ...
     */
	@Autowired
    public Neo4jService(CrudNeo4jDriver driver, Neo4jOptByUser optByUserSevice, RuleDomain rule, BeanShellService bss) {
        this.driver=driver;
...
//    }
    ...
    public void removeBy(Long id, String label) {
	   try {
	       String delRel = "MATCH(n:"+label+")-[r]-(m) where id(n)="+id+" delete r ";
	       execute(delRel);
	       String cypher = "MATCH(n:"+label+") where id(n)="+id+" delete n ";
		execute(cypher);
	    } catch (Exception e) {
		LoggerTool.debug(logger,"Excetion : ", e);
		debugLog.debug("Excetion : ", e);
    ...
    public void delete(String key,String value, String label) {
	   try {
	       String delRel = "MATCH(n:"+label+") where n."+key+"='"+value+"' delete n ";
	       execute(delRel);
	    } catch (Exception e) {
		LoggerTool.debug(logger,"Excetion : ", e);
		debugLog.debug("Excetion : ", e);
    ...
    public void delete(Long id) {
	   try {
	       execute("MATCH(n) where id(n)="+id +" delete n");
	    } catch (Exception e) {
		LoggerTool.debug(logger,"Excetion : ", e);
		debugLog.debug("Excetion : ", e);
	    }
	...
	if (isNeo4jConnectionUp()) {
		...
		LoggerTool.info(logger,"\n execute: \n" + query +"\n==return======>>>>\n" + listMapString(queryData));
			...
			LoggerTool.error(logger,"Excetion : ", e);
		}
	} else {
	    LoggerTool.debug(logger,"Driver or Session is down, check the configuration");
	    debugLog.debug("Driver or Session is down, check the configuration");
	...
	public void deleteRel(String endlabel, Long id) {
		String cypher=" MATCH(s)-[r]->(e:"+endlabel+")  where id(s)="+id+" delete r";
	}
}
