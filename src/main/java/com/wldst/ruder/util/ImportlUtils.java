 ...
 *
 */
public class ImportlUtils {

    public static final String OFFICE_EXCEL_2003_POSTFIX = "xls";
    public static final String OFFICE_EXCEL_2010_POSTFIX = "xlsx";
    public static final String TSV_POSTFIX = "tsv";
    public static final String CSV_POSTFIX = "csv";
    public static final String EMPTY = "";
    public static final String POINT = ".";
    private static final Logger logger = LoggerFactory.getLogger(ImportlUtils.class);
     ...
     * @throws IOException
     */
    public static List<Map<String, String>> readExcel(File file, int startRowNumber, int sheetNum,
	    Map<Integer, String> fieldMap) throws IOException {
	String originalFileName = file.getName();
	if (!originalFileName.endsWith(CruderConstant.FILE_TYPE_XLS)
		&& (!originalFileName.endsWith(CruderConstant.FILE_TYPE_XLSX))) {
    ...
    public static List<List<String>> readData(String name,InputStream input) throws IOException {
	if (name == null || EMPTY.equals(name.trim())) {
	    LoggerTool.info(logger,"name is EMPTY or Null");
	    return null;
	} else {
	    String postfix = getPostfix(name);
	    LoggerTool.info(logger,"name",postfix);
	    if (!EMPTY.equals(postfix)) {
		if (OFFICE_EXCEL_2003_POSTFIX.equals(postfix)) {
		    LoggerTool.info(logger,"OFFICE_EXCEL_2003_POSTFIX:{}",postfix);
		    return readXls(input);
		} else if (OFFICE_EXCEL_2010_POSTFIX.equals(postfix)) {
		    LoggerTool.info(logger,"OFFICE_EXCEL_2010_POSTFIX:{}",postfix);
		    return readXlsx(input);
		}else if (TSV_POSTFIX.equals(postfix)) {
		    LoggerTool.info(logger,"TSV_POSTFIX:{}",postfix);
		    return FileOpt.readfile(input,"\t");
		}else if (CSV_POSTFIX.equals(postfix)) {
		    LoggerTool.info(logger,"CSV_POSTFIX:{}",postfix);
		    return FileOpt.readfile(input,",");
		} else {
		    return null;
		}
    ...
    private static List<List<String>> readXlsx(InputStream input) {
	int totalRows; // sheet中总行数
	int totalCells; // 每一行总单元格数
	List<List<String>> list = new ArrayList<>();
    ...
    public static List<List<String>> readXls(InputStream input) {
	int totalRows; // sheet中总行数
	int totalCells; // 每一行总单元格数
	List<List<String>> list = new ArrayList<>();
	// IO流读取文件
