...
public class CusCompiler {
  ...
  static class SpringBootJarFileManager implements JavaFileManager {
	 ...
	 * 
	 * public MemoryClassLoader(Map<String, byte[]> classBytes, ClassLoader
	 * classLoader) { super(new URL[0], classLoader);
	 * this.classBytes.putAll(classBytes); }
	 * 
	 * @Override protected Class<?> findClass(String name) throws
	 * ClassNotFoundException { System.out.println("findClass: " + name); byte[] buf
	 * = classBytes.get(name); if (buf == null) { return super.findClass(name); }
  ...
  public Map<String, byte[]> compile(String className, String code) throws Exception {
    ...
    for (String key : springBootJarFileManager.getClassBytes().keySet()) {
      LoggerTool.info(logger,"class: " + key + " len: " + Integer.valueOf(springBootJarFileManager.getClassBytes().get(key).length).toString());
    }
