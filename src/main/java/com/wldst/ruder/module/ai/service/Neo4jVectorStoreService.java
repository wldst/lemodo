...
public class Neo4jVectorStoreService extends MapTool {
    ...
    @Autowired
    public Neo4jVectorStoreService(OllamaEmbeddingService embeddedService, 
	    ConfigService configService,GoodsService goodsService) {
	this.ollamaEmbeddingService = embeddedService;
	this.configService = configService;
	this.goodsService = goodsService;
	try {
	    if (this.goodsService!=null&&this.goodsService.isInited()) {
		init();
	    }
	...
	try {
	    ...
	    if (neo4jVectorConfig == null||neo4jVectorConfig.isEmpty()) {
	    ...
	    if (StringUtils.isNotBlank(neo4jUri) && StringUtils.isNotBlank(neo4jUser)
		    && StringUtils.isNotBlank(neo4jPassword)) {
	...
	try (Session session = driver.session()) {
	    ...
	    Result result = session
		    .readTransaction(tx -> tx.run(cypher, Values.parameters("queryVector", queryVector, "topK", topK)));
