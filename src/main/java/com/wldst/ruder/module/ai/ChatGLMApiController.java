 ...
 */
@Controller
@RequestMapping("${server.context}/chatglm")
public class ChatGLMApiController extends GoodsDomain{
    final static Logger logger=LoggerFactory.getLogger(ChatGLMApiController.class);
    @Autowired
    ...
    @ResponseBody
    public WrappedResult cypher(@RequestBody JSONObject params)
            throws DefineException, EvalError{
        List<Map<String, Object>> cypherData=neo4jService.cypher(params.getString("cypher"));
        return ResultWrapper.wrapResult(false, cypherData, null, EXECUTE_SUCCESS);
    }

    //    @RequestMapping(value = "/cypherQuery", method = {RequestMethod.POST,
//            RequestMethod.GET}, produces = "application/json;charset=UTF-8")
//    @ResponseBody
//    public WrappedResult cypherQuery(@RequestBody JSONObject vo)
//            throws DefineException, EvalError {
//        List<Map<String, Object>> cypherData=neo4jService.cypher(vo.getString("cypher"));
//
//        return ResultWrapper.wrapResult(false, cypherData, null, EXECUTE_SUCCESS);
//    }
    ...
    public WrappedResult talk(@RequestBody JSONObject vo)
        ...
        Long sessionId=getSessionId(myId, msg);
//        ChatSession
        msg=msg.replaceAll("\n", "");
        ...
        //处理需求
        WrappedResult developData=processNeed(msg, sessionId, myId);
        if(developData!=null) return developData;
        //元数据解析并更新环境
        WrappedResult metaDataMaintain=agentProcessMsg(msg, sessionId, chatContextId);
        //ChatSession
        if(metaDataMaintain!=null&&metaDataMaintain.getStatus()&&metaDataMaintain.getData()!=null)
            return metaDataMaintain;

        //如果是开发请求，新建一个开发进程，并且返回一个进程id，根据进程id，可以获取进程状态，进程结果，进程进度等
        //启动代理处理进程
        // 开发事情判断
        developData=developSomeThing(msg, sessionId, myId);
        //ChatSession
        if(developData!=null) return developData;
        ...
        //关联系统元数据，添加元数据
        WrappedResult crudAnswer=crudProcess(msg, sessionId, chatContextId, data, myId);
        if(crudAnswer!=null) return crudAnswer;
        // Map<String,Object> postData = post(chatUrl,data);
        return aiService.loaclAI(data, sessionId);
    }

    @NotNull
    private Long getSessionId(String myId, String msg){
    ...
    @Nullable
    private WrappedResult crudProcess(String msg, Long sessionId, String chatContextId, Map<String, Object> data, String myId) throws EvalError{
        String message="Agent:NounName:"+msg;
        WrappedResult entitData=agentProcessMsg(message, sessionId, chatContextId);
    ...
    private WrappedResult agentProcessMsg(String msg1, Long sessionId, String chatContextId) throws EvalError{
        ...
        if(msg.startsWith("Agent")){
            ...
            // 尝试根据名称或代码获取智能体的相关信息。
            Map<String, Object> agentData=neo4jService.getAttMapBy(CODE, talks[1].trim(), "Agent");
            if(agentData==null){
                agentData=neo4jService.getAttMapBy(NAME, talks[1].trim(), "Agent");
            }
            ...
            // 进行智能体的工作处理，包括回答问题等。
            answer=agentWork(userChat, agentData, talks[2], sessionId, chatContextId);
    ...
    @Nullable
    private WrappedResult developSomeThing(String msg, Long sessionId, String myId) throws EvalError{
        if((msg.startsWith("开发")||msg.startsWith("设计"))&&(msg.endsWith("程序")||msg.endsWith("工具")||msg.endsWith("应用")||msg.endsWith("系统")||msg.endsWith("功能"))){
            ...
            if(flowInstance.size()>0){
                ...
                while(status!=null&&status!=WFEConstants.WFSTATUS_END&&status!=WFEConstants.WFSTATUS_PAUSE){
                    ...
                    String content=validAnswer(param, answer);
     ...
     * @throws EvalError
     */
    private WrappedResult processNeed(String msg, Long sessionId, String myId) throws EvalError{
...
//            String[] querys = msg.split("要");
            ...
            Map<String, Object> datai=creatDataFlow(msg, label);
            Map<String, Object> answer=null;
    ...
    @Nullable
    private String validAnswer(Map<String, Object> param, Map<String, Object> answer){
        String content=content(answer);
        if(content(answer).endsWith("请检查智能体是否在线")){
            answer=aiService.onlineCallLLM(param);
...
//        messages.add(agent);
        ...
        Map<String, Object> answer=aiService.callAI(param);
        if(answer!=null&&!answer.isEmpty()){
            if(answer.containsKey("content")){
                answer.put("role", "assistant");
                return answer;
        ...
        //判断chatUrl地址是否正常
        return aiService.localAI(currentTask, param);
    }
    ...
    private Map<String, Object> agentWork(JSONObject m2, Map<String, Object> angetData, String msg, Long sessionId, String chatContextId){
        ...
        Map<String, Object> answer=null;
        if("ollama".equals(llmCallKey)&&ollamaModel.startsWith("deepseek")){
            Map<String, Object> data=aiService.getDeepseekParam();
           String prompt= content(agent).replace("${UserInput}",msg);
            data.put("prompt", prompt);
            data.put("model", ollamaModel);
            answer=aiService.callAI(data);
            if(answer!=null&&!answer.isEmpty()){
                if(answer.containsKey("response")){
                    String response = string(answer, "response");
                    aiService.addChatItem(sessionId, answer);
    ...
    private Map<String, Object> creatDataFlow(String msg, String label){
        Map<String, Object> data=new HashMap<>();
        if(msg.length()>20){
            data.put("name", msg.substring(0, 20));
        }else{
    ...
    @NotNull
    private JSONObject packageChatByAgent(Map<String, Object> agent){
